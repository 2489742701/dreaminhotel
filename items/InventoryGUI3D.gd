...
# 背包界面主脚本 - 管理游戏背包界面，显示物品列表、处理选择和装备功能
# CanvasLayer确保UI始终显示在最上层
extends CanvasLayer
...
func refresh():
	...
	for i in range(inv.items.size()):
		...
		# 设置按钮的文本为物品名称
		item_button.text = item.item.name_tr_key
		...
		match item.item.kind:
			...
			_: # 其他类型物品（装备类）
				if item.item.kind == Item.Kind.WEAPON:  # 检查物品是否可装备
...
func _equip_text(idx: int) -> String:
	return "卸下" if idx == inv.equipped_id else "装备"
